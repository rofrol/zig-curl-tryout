...
fn post(allocator: Allocator, easy: Easy) !void {
    ...
    // sometimes I need to use stdout separately, otherwise stringify does not print what it should.
    // it prints beginning of parsed.value, instead of parsed.value.json.
    // const stdout = std.io.getStdOut().writer();
    // try stdout.writeByte('\n');

    try std.json.stringify(&parsed.value.json, .{}, std.io.getStdOut().writer());
